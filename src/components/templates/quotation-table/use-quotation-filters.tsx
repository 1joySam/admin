...
type QuotationDateFilter = null | {
  gt?: string
  lt?: string
}

type QuotationFilterAction =
  | { type: "setQuery"; payload: string | null }
  | { type: "setFilters"; payload: QuotationFilterState }
  | { type: "reset"; payload: QuotationFilterState }
  | { type: "setOffset"; payload: number }
  | { type: "setDefaults"; payload: QuotationDefaultFilters | null }

interface QuotationFilterState {
  query?: string | null
  isDynamic: {
    open: boolean
    filter: null | string[]
  ...
  date: {
    open: boolean
    filter: null | QuotationDateFilter
  }
  limit: number
  offset: number
  additionalFilters: QuotationDefaultFilters | null
}
...
const formatDateFilter = (filter: QuotationDateFilter) => {
  if (filter === null) {
    return filter
  }
...
const reducer = (
  state: QuotationFilterState,
  action: QuotationFilterAction
...
export const useQuotationFilters = (
  existing?: string,
  defaultFilters: quotationDefaultFilters | null = null
) => {
  ...
  const initialTabs = useMemo(() => {
    const storageString = localStorage.getItem("quotations::filters")
    if (storageString) {
  ...
  const setDefaultFilters = (filters: QuotationDefaultFilters | null) => {
  ...
  const setFilters = (filters: QuotationFilterState) => {
      ...
      if (key === "query") {
        ...
        if (key === "date") {
          toQuery[stateFilterMap[key]] = formatDateFilter(
            value.filter as QuotationDateFilter
          )
  ...
  const getRepresentationObject = (fromObject?: QuotationFilterState) => {
  ...
  const saveTab = (tabName: string, filters: QuotationFilterState) => {
    ...
    const storedString = localStorage.getItem("quotations::filters")
    ...
    if (existing) {
      ...
      localStorage.setItem("quotations::filters", JSON.stringify(existing))
    } else {
      const newFilters = {}
      newFilters[tabName] = repString
      localStorage.setItem("quotations::filters", JSON.stringify(newFilters))
    }
...
const parseQueryString = (
  queryString?: string,
  additionals: QuotationDefaultFilters | null = null
): QuotationFilterState => {
  const defaultVal: QuotationFilterState = {
    date: {
      open: false,
      filter: null,
    },
